# Tax Polonica — gotowe teksty „Przenieś księgowość do nas"

Motyw przewodni: **nie musisz czekać do końca roku — biuro rachunkowe można zmienić w każdym miesiącu, a my przeprowadzimy Cię przez formalności.**

Dane firmy używane w tekstach:
- strona: **www.taxpolonica.pl**
- e-mail: **kontakt@taxpolonica.pl**
- obszar: **Wrocław + obsługa zdalna w całej Polsce**

> Uwaga: w miejscach oznaczonych `[…]` uzupełnij własne dane (telefon, adres, godziny, ceny).
> Zanim opublikujesz, sprawdź zgodność obietnic z Twoją umową i cennikiem — teksty są propozycją marketingową, nie poradą prawną.

---

## 1. Wizytówka Google (Profil Firmy) — najważniejsze, bo darmowe

### Nazwa firmy
```
Tax Polonica — biuro rachunkowe
```
(Nie dopisuj do nazwy słów kluczowych typu „Wrocław tanie księgowość" — Google zawiesza takie wizytówki. Miasto wynika z adresu.)

### Kategoria główna
```
Biuro rachunkowe
```
### Kategorie dodatkowe (dodaj 2–4)
```
Doradca podatkowy
Usługi księgowe
Biuro kadrowo-płacowe
```

### Opis firmy (do 750 znaków — wersja gotowa do wklejenia)
```
Tax Polonica to biuro rachunkowe z Wrocławia, które obsługuje klientów w całej Polsce — online, bez wizyt w biurze.

Prowadzimy KPiR, ryczałt, pełne księgi, VAT, JPK, kadry i płace oraz ZUS. Obsługujemy jednoosobowe działalności, spółki, fundacje i stowarzyszenia.

Zmieniasz biuro rachunkowe? Nie musisz czekać do końca roku — księgowość przejmujemy w trakcie roku, w dowolnym miesiącu. Pomagamy odzyskać dokumenty od poprzedniego biura, przygotowujemy wypowiedzenie, przenosimy dane i pełnomocnictwa. Formalności bierzemy na siebie.

Pierwsza rozmowa i wycena są bezpłatne.

Napisz: kontakt@taxpolonica.pl
Więcej: www.taxpolonica.pl
```

### Usługi w wizytówce (dodaj każdą osobno — Google indeksuje je jako osobne frazy)
| Nazwa usługi | Opis (do wklejenia) |
|---|---|
| Przejęcie księgowości od innego biura | Przejmujemy księgowość w trakcie roku — w dowolnym miesiącu. Odbieramy dokumenty, sprawdzamy zaległości, przenosimy dane i pełnomocnictwa. Formalności po naszej stronie. |
| Księgowość dla jednoosobowej działalności | KPiR, ryczałt, VAT, JPK, ZUS. Stała opieka księgowej i jasna cena bez niespodzianek. |
| Księgowość dla spółek | Pełne księgi rachunkowe, sprawozdania finansowe, CIT, JPK CIT, e-sprawozdania do KRS. |
| Księgowość dla fundacji i stowarzyszeń | Sprawozdawczość NGO, działalność statutowa i gospodarcza, załączniki 4–7. |
| Kadry i płace | Umowy, listy płac, ZUS, PIT-11, e-akta pracownicze. |
| Rozliczenia roczne PIT/CIT | Roczne zeznania dla firm i osób prywatnych, ulgi i korekty. |

### Sekcja „Pytania i odpowiedzi" (zadaj je sama z drugiego konta i odpowiedz — to legalne i bardzo poprawia widoczność)
1. **Czy mogę zmienić biuro rachunkowe w trakcie roku?**
   Tak. Przepisy nie każą czekać do 31 grudnia — księgowość można przenieść w dowolnym miesiącu. Trzeba tylko zachować okres wypowiedzenia z umowy i zebrać dokumenty za bieżący rok. Pomagamy w obu krokach.
2. **Ile trwa przeniesienie księgowości?**
   Zwykle kilka dni roboczych od momentu, gdy dostaniemy dokumenty i dostępy. Najlepiej startować od 1. dnia miesiąca.
3. **Czy obsługujecie firmy spoza Wrocławia?**
   Tak, pracujemy zdalnie z klientami z całej Polski. Dokumenty przesyłasz elektronicznie.
4. **Ile kosztuje obsługa?**
   Cena zależy od formy działalności i liczby dokumentów. Wycena jest bezpłatna — napisz na kontakt@taxpolonica.pl.

### 3 gotowe posty na wizytówkę (wrzucaj 1 co tydzień — posty podbijają pozycję)

**Post 1 — główny przekaz**
```
Nie musisz czekać do końca roku, żeby zmienić biuro rachunkowe.
Księgowość przejmujemy w trakcie roku — w dowolnym miesiącu.
Odbierzemy dokumenty od poprzedniego biura, sprawdzimy zaległości i przeniesiemy dane. Formalności bierzemy na siebie.
Wrocław i cała Polska online. Wycena bezpłatna: kontakt@taxpolonica.pl
```
**Post 2 — na obiekcję „to skomplikowane"**
```
„Zmiana księgowej to pewnie masa papierów"? Po Twojej stronie są 3 rzeczy:
1) wypowiedzenie umowy (przygotujemy wzór),
2) odbiór dokumentów za bieżący rok,
3) podpisanie nowego pełnomocnictwa.
Resztę robimy my. Napisz: kontakt@taxpolonica.pl
```
**Post 3 — sygnały, że czas zmienić biuro**
```
Czas zmienić biuro rachunkowe, jeśli:
• nie odbierają telefonu, gdy masz kontrolę lub termin,
• o zmianach w przepisach dowiadujesz się z internetu, nie od księgowej,
• faktura co miesiąc inna niż umowa,
• zapłaciłeś odsetki za czyjeś przeoczenie.
Przejmujemy księgowość w trakcie roku. www.taxpolonica.pl
```

---

## 2. Teksty na stronę www.taxpolonica.pl (podstrona „Zmiana biura rachunkowego")

> Zrób osobną podstronę pod adresem np. `www.taxpolonica.pl/zmiana-biura-rachunkowego`.
> Jedna podstrona = jedna fraza w Google. To działa lepiej niż dopisanie akapitu na stronie głównej.

### Tytuł strony (title — max ~60 znaków)
```
Zmiana biura rachunkowego w trakcie roku | Wrocław i online
```
### Opis (meta description — max ~155 znaków)
```
Przenieś księgowość do Tax Polonica — nie musisz czekać do końca roku. Przejmujemy księgi w dowolnym miesiącu. Wrocław i cała Polska. Wycena gratis.
```

### Nagłówek główny (H1)
```
Przenieś księgowość do nas — nie musisz czekać do końca roku
```
### Zdanie pod nagłówkiem
```
Biuro rachunkowe można zmienić w dowolnym miesiącu. Przejmiemy Twoje księgi w trakcie roku, odbierzemy dokumenty od poprzedniego biura i załatwimy formalności za Ciebie. Wrocław, a klienci — z całej Polski.
```
### Przyciski
```
[ Napisz do nas: kontakt@taxpolonica.pl ]    [ Sprawdź, jak wygląda przeniesienie ]
```

### Sekcja: „Znasz to?" (H2)
```
Znasz to uczucie?
```
- Dzwonisz do księgowej przed terminem i nikt nie odbiera.
- Pytasz o coś prostego, a odpowiedź przychodzi za tydzień — albo wcale.
- O zmianach w przepisach dowiadujesz się od znajomego, nie od swojego biura.
- Zapłaciłeś odsetki albo mandat za cudze przeoczenie.
- Co miesiąc inna faktura, a w umowie była jedna kwota.
- Boisz się zapytać o coś drugi raz, żeby nie wyjść na niekompetentnego.

```
Jeśli choć jedno się zgadza — nie musisz z tym żyć do grudnia.
```

### Sekcja: „Najczęstszy mit" (H2)
```
„Księgowość zmienia się tylko od stycznia" — to nieprawda
```
```
Żaden przepis nie każe Ci czekać do końca roku podatkowego. Księgi można przekazać nowemu biuru w trakcie roku — liczy się tylko to, żeby zachować okres wypowiedzenia z dotychczasowej umowy i skompletować dokumentację za bieżący rok. Najwygodniejszy moment to pierwszy dzień miesiąca, a jeszcze lepiej — pierwszy dzień kwartału.

Im dłużej zwlekasz, tym więcej miesięcy do sprawdzenia i odtworzenia. Każdy miesiąc zwłoki to więcej pracy, nie mniej.
```

### Sekcja: „Jak to wygląda krok po kroku" (H2)
```
Przeniesienie księgowości — 5 kroków, w których nie zostajesz sam
```
1. **Bezpłatna rozmowa (ok. 20 minut).** Mówisz, czym się zajmujesz i jak dziś wygląda Twoja księgowość. Dostajesz konkretną cenę — na piśmie.
2. **Wypowiedzenie dotychczasowej umowy.** Dajemy gotowy wzór i podpowiadamy datę, żeby nie zostać bez obsługi ani jednego dnia.
3. **Odbiór dokumentów.** Ustalamy listę tego, co musisz dostać od poprzedniego biura: księgi i ewidencje za bieżący rok, ewidencje VAT i wysłane JPK, rejestry środków trwałych, dokumenty kadrowe i ZUS, potwierdzenia wysyłek deklaracji.
4. **Przejęcie i sprawdzenie.** Wczytujemy dane, uzgadniamy salda, sprawdzamy, czy nie zostały zaległości i nierozliczone korekty. Jeśli coś jest nie tak — mówimy wprost i proponujemy naprawę.
5. **Pełnomocnictwa i start.** Podpisujesz nowe pełnomocnictwo do e-deklaracji, ustawiamy dostępy (ZUS PUE, KSeF, e-Urząd Skarbowy) i przejmujemy bieżące terminy. Od tego momentu pilnujemy ich my.

```
Twoja rola: jedna rozmowa, jeden podpis i przekazanie dokumentów. Resztę robimy my.
```

### Sekcja: „Co dostajesz" (H2)
```
Czego możesz się po nas spodziewać
```
- **Odbierany telefon i odpisany mail** — odpowiadamy w ciągu 24 godzin roboczych.
- **Jedna, stała cena** — ustalona z góry, bez dopłat „za pytanie".
- **Księgowa, która tłumaczy po ludzku** — dostajesz odpowiedź, a nie numer paragrafu.
- **Pilnowanie terminów za Ciebie** — przypominamy zanim minie, nie po fakcie.
- **Pełna obsługa online** — dokumenty wysyłasz elektronicznie, skądkolwiek jesteś.
- **Doświadczenie w trudniejszych tematach** — fundacje i stowarzyszenia, spółki, JPK CIT.

### Sekcja: „Dla kogo" (H2)
```
Kogo obsługujemy
```
- **Jednoosobowe działalności** — KPiR, ryczałt, VAT, ZUS, JPK.
- **Spółki** — pełne księgi, CIT, JPK CIT, sprawozdania finansowe i e-sprawozdania do KRS.
- **Fundacje i stowarzyszenia** — działalność statutowa i gospodarcza, sprawozdawczość NGO.
- **Firmy zatrudniające** — kadry, płace, ZUS, e-akta.

### Sekcja: FAQ (H2) — ważna dla Google, bo trafia do „rozwijanych pytań"
```
Najczęstsze pytania o zmianę biura rachunkowego
```
**Czy naprawdę mogę zmienić księgowość w środku roku?**
Tak. Nie ma przepisu, który każe czekać do stycznia. Trzeba jedynie dotrzymać okresu wypowiedzenia z dotychczasowej umowy i przekazać dokumentację za bieżący rok.

**Kiedy najlepiej to zrobić?**
Od pierwszego dnia miesiąca, a najwygodniej od początku kwartału. Ustalamy datę tak, żeby żaden termin nie wypadł „pomiędzy" biurami.

**Co, jeśli poprzednie biuro nie chce oddać dokumentów?**
Dokumenty źródłowe są Twoją własnością i biuro ma obowiązek je zwrócić. Podpowiemy, jak sformułować wezwanie i co zrobić, gdy nie ma reakcji.

**A jeśli w moich księgach jest bałagan albo zaległości?**
To częstszy powód zmiany niż myślisz. Najpierw robimy przegląd i mówimy wprost, co wymaga naprawy — z wyceną, zanim cokolwiek zaczniemy.

**Czy muszę przyjeżdżać do Wrocławia?**
Nie. Pracujemy zdalnie z klientami z całej Polski. Jeśli wolisz spotkanie osobiste — jesteśmy we Wrocławiu.

**Ile to kosztuje?**
Cena zależy od formy działalności, liczby dokumentów i liczby pracowników. Wycena jest bezpłatna i niezobowiązująca.

**Czy w trakcie zmiany coś przestanie działać?**
Nie — o to właśnie chodzi w planowaniu daty. Ustalamy przekazanie tak, żeby deklaracje, JPK i ZUS szły bez przerwy.

### Sekcja końcowa (CTA)
```
Zacznij od jednej wiadomości

Napisz, czym się zajmujesz i jak dziś wygląda Twoja księgowość.
Odpowiemy w ciągu 24 godzin roboczych — z konkretną ceną i terminem przejęcia.

kontakt@taxpolonica.pl
www.taxpolonica.pl
Wrocław — obsługujemy klientów w całej Polsce.
```

---

## 3. Krótkie wersje (media społecznościowe, ogłoszenia, podpis w mailu)

### Facebook / LinkedIn / grupy przedsiębiorców (post)
```
Mit, który kosztuje przedsiębiorców nerwy: „biuro rachunkowe zmienia się tylko od stycznia".

Nieprawda. Księgowość można przenieść w dowolnym miesiącu — wystarczy zachować okres wypowiedzenia i skompletować dokumenty za bieżący rok.

W Tax Polonica przejmujemy księgi w trakcie roku: przygotowujemy wypowiedzenie, pomagamy odebrać dokumenty od poprzedniego biura, sprawdzamy zaległości i przejmujemy terminy.

Wrocław, obsługa online w całej Polsce. Wycena bezpłatna.
📧 kontakt@taxpolonica.pl
🌐 www.taxpolonica.pl
```

### Ogłoszenie (OLX / grupy lokalne) — krótko
```
Biuro rachunkowe Tax Polonica — przejmiemy Twoją księgowość w trakcie roku

Nie musisz czekać do końca roku, żeby zmienić księgową. Przejmujemy księgi w dowolnym miesiącu i załatwiamy formalności za Ciebie: wypowiedzenie, odbiór dokumentów, przeniesienie danych i pełnomocnictw.

KPiR, ryczałt, pełne księgi, VAT, JPK, kadry i płace, ZUS. JDG, spółki, fundacje i stowarzyszenia.

Wrocław + cała Polska online. Pierwsza wycena bezpłatna.
kontakt@taxpolonica.pl | www.taxpolonica.pl
```

### Podpis w e-mailu
```
Tax Polonica — biuro rachunkowe
Przejmujemy księgowość w trakcie roku — nie trzeba czekać do końca roku.
Wrocław | obsługa online w całej Polsce
kontakt@taxpolonica.pl | www.taxpolonica.pl
```

### Wizytówka papierowa / ulotka (awers i rewers)
```
AWERS
Tax Polonica
Biuro rachunkowe
Wrocław · obsługa online w całej Polsce
kontakt@taxpolonica.pl
www.taxpolonica.pl
[ telefon ]

REWERS
Przenieś księgowość do nas.
Nie musisz czekać do końca roku.
Przejmujemy księgi w dowolnym miesiącu:
• przygotujemy wypowiedzenie umowy
• pomożemy odebrać dokumenty
• sprawdzimy zaległości
• przejmiemy terminy i pełnomocnictwa
Wycena bezpłatna.
```

---

## 4. Frazy, na które warto celować (używaj ich naturalnie w tekstach)

Najłatwiejsze do zdobycia za darmo (mała konkurencja, konkretna intencja):
- zmiana biura rachunkowego w trakcie roku
- jak zmienić biuro rachunkowe
- przeniesienie księgowości do innego biura
- biuro rachunkowe Wrocław przejęcie księgowości
- księgowość online cała Polska
- biuro rachunkowe dla fundacji / dla stowarzyszenia
- biuro rachunkowe Wrocław opinie

Nie walcz na starcie o „biuro rachunkowe Wrocław" — tam wygrywają duzi i płatne reklamy. Wygrywasz na pytaniach, które ludzie faktycznie wpisują, gdy mają problem.
